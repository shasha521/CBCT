...
parser.add_argument('--result_dir', default='./results2/tv1loss/deblurring/', type=str, help='Directory for results')
parser.add_argument('--dataset', default='input,target', type=str, help='Test Dataset') 
parser.add_argument('--weights', default='/home/user/space/sjr/Uformer-main/model_best.pth',
...
def proc(filename):
    tar,prd = filename
    # tar_img= Image.open(tar).convert('RGB')
    tar_img=cv2.imread(tar)
    # tar_img = io.imread(tar_img)
    prd_img = cv2.imread(prd)
...
if __name__ == "__main__":
    ...
    rgb_dir_test = os.path.join(args.input_dir, 'test')
    test_dataset = get_validation_data(rgb_dir_test)
    ...
    file_path = os.path.join(args.result_dir, args.arch)
    ...
    with torch.no_grad():
        for ii, data_test in enumerate(tqdm(test_loader), 0):
            ...
            psnr = psnr_loss(restored, rgb_gt)
            ssim = ssim_loss(restored, rgb_gt, multichannel=True)
            psnr_val_rgb.append(psnr)
            ssim_val_rgb.append(ssim)
            with open(os.path.join(file_path,'psnr_ssim.txt'),'a') as f:
                f.write(filenames[0]+'PSNR: {:f} SSIM: {:f}'.format(psnr, ssim)+'\n')
    ...
    with open(os.path.join(file_path,'psnr_ssim.txt'),'a') as f:
                f.write('zong'+'PSNR: {:f} SSIM: {:f}'.format(avg_psnr, avg_ssim)+'\n')
    # ## evaluate
    # results = {}
   

    # file_path = os.path.join(args.result_dir, args.arch)
    # gt_path = os.path.join(args.input_dir,'test', 'target')
